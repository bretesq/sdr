...
# EVERYTHING below lives in main(), called on the last line.
#
# bash reads a script incrementally, so replacing this file while a run is in
# flight makes the running shell resume at a byte offset that no longer means
# what it did — a 25-minute run ended in "syntax error near unexpected token"
# after deploying an update mid-search. Wrapping the body in a function forces
# bash to parse the whole definition before executing any of it, so the file can
# be overwritten safely once the run has started.
main() {
