...
| `lwin_active_whitelist.txt` | One bare tgid per line. **Regenerated by `scripts/lwin_listen.sh:75` (via `make_whitelist.py`) on every recording run**, so its size tracks whichever preset was last used — 601 for the full BR-area set, 52 for `--preset pd --include-partial`. Treat the count as runtime state, never as a fixture. |
...
Expected: **11 passed**. Four of these assert against the live DB (4163 entries, ascending tgid, 601 BR-area), so a passing run is also the real-data verification — no separate step needed.
...
```bash
pgrep -af '[l]win_listen|[r]x\.py|[u]dp_audio_record|[s]tt_watch' || echo "clean"
```
...
4. The footer's whitelist count matches `wc -l < lwin_active_whitelist.txt` **as it stands right now** — not a fixed 601. The file is rewritten by every recording run, so it is 601 only until the first session with a narrower preset.
...
- Stop → idle, and `pgrep -af '[l]win_listen|[r]x\.py|[u]dp_audio_record|[s]tt_watch'` prints nothing.
...
Expected: **4 + 8 + 11 + 7 = 30 passed** across `paths`, `files`, `talkgroups`, `processes`.
...
Expected: build clean, both routes answer, **30 tests pass**, and `web/` shows only gitignored sidecar files.
...
- [ ] `./node_modules/.bin/vitest run` → **30 passed**
...
- [ ] The footer's whitelist count matches `wc -l < lwin_active_whitelist.txt` **at that moment**. Do not hard-code 601: `lwin_listen.sh` regenerates the file on every run, so it is 601 only before the first narrower-preset session.
